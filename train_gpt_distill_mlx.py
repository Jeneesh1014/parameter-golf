...
class Hyperparameters:
    ...
    # ─── DISTILLATION: Teacher model size ───
    teacher_num_layers: int = int(
        os.environ.get("TEACHER_NUM_LAYERS", "12")
    )
    teacher_model_dim: int = int(
        os.environ.get("TEACHER_MODEL_DIM", "768")
    )
    teacher_num_heads: int = int(
        os.environ.get("TEACHER_NUM_HEADS", "12")
    )
    teacher_num_kv_heads: int = int(
        os.environ.get("TEACHER_NUM_KV_HEADS", "4")
    )

    # ─── DISTILLATION: Temperature and Alpha ───
    temperature: float = float(
        os.environ.get("TEMPERATURE", "4.0")
    )
    alpha: float = float(
        os.environ.get("ALPHA", "0.5")
    )
...
# Combines real-label loss + teacher-guided loss
# ─────────────────────────────────────────────
def distillation_loss(
    ...
    """
    student_logits : shape [N, vocab_size]  (student predictions)
    teacher_logits : shape [N, vocab_size]  (teacher predictions)
    targets        : shape [N]              (real next tokens)
    temperature    : float  (higher = softer distributions)
    alpha          : float  (weight for CE loss, 0 to 1)
    """

    # ── LOSS 1: Student learns from real labels ──
    ...
    # ── LOSS 2: Student learns from teacher ──
    # Divide logits by temperature BEFORE softmax
    # This makes the distribution "softer"
    # Temperature 1.0 = sharp, 4.0 = soft and spread out
    student_soft = mx.softmax(
        student_logits / temperature,
        axis=-1
    )
    teacher_soft = mx.softmax(
        teacher_logits / temperature,
        axis=-1
    )

    # KL Divergence: measures how different
    # student distribution is from teacher
    # Formula: sum( teacher * log(teacher/student) )
    kl_loss = mx.mean(
        mx.sum(
            teacher_soft * (
                mx.log(teacher_soft + 1e-8) -
                mx.log(student_soft + 1e-8)
            ),
            axis=-1
        )
    )

    # Multiply by T^2 (Hinton et al. 2015 paper)
    # This keeps gradient magnitudes balanced
...
class GPT(nn.Module):
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array, teacher_logits:mx.array | None =None, temperature:float=4.0, alpha:float =0.5) -> mx.array:
        ...
        # ── PATH A: No teacher — original behavior unchanged ────
        ...
        # ── PATH B: Teacher provided — distillation ─────────────
        # Get student logits (all at once — simpler for distillation)
        student_logits_proj = x @ self.tok_emb.weight.astype(x.dtype).T
        student_logits = self.softcap(student_logits_proj).astype(mx.float32)

        # teacher_logits already computed outside (no grad)
        # Call the standalone distillation_loss function
...
def distill_step(
    ...
    One distillation step:
      1. Read tokens ONCE (shared between teacher and student)
      2. Train teacher on real data (normal CE loss)
      3. Get teacher logits for those same tokens
      4. Train student with distillation loss
    
    Returns:
      teacher_loss_value : float tensor
      student_loss_value : float tensor
      teacher_grads      : gradient dict for teacher
      student_grads      : gradient dict for student
    """
    ...
    teacher_loss_accum = mx.array(0.0, dtype=mx.float32)
    student_loss_accum = mx.array(0.0, dtype=mx.float32)
    teacher_grad_accum: dict[str, mx.array] | None = None
    student_grad_accum: dict[str, mx.array] | None = None

    for chunk_tokens in chunk_sizes:
        # ── READ TOKENS ONCE ──────────────────────────────────
        # Both teacher and student use SAME x, y
        x, y = train_loader.next_batch(chunk_tokens, args.train_seq_len)
        scale = float(y.size) / total_tokens

        # ── STEP 1: Teacher learns from real data ─────────────
        # Teacher uses normal CE loss (same as original training)
        t_loss, t_grads = compiled_teacher_loss_and_grad(x, y)
        teacher_loss_accum = teacher_loss_accum + t_loss.astype(mx.float32) * scale
        teacher_grad_accum = accumulate_flat_grads(teacher_grad_accum, t_grads, scale)

        # ── STEP 2: Get teacher logits (no gradient) ──────────
        # We run teacher forward pass to get its predictions
        # mx.stop_gradient ensures teacher logits dont affect
        # student gradient computation
        teacher_hidden = compiled_teacher_forward(x)
        t_hidden_flat = teacher_hidden.reshape(-1, args.teacher_model_dim)

        # Project teacher hidden → logits using teacher embedding
        t_logits_proj = t_hidden_flat @ teacher.tok_emb.weight.astype(
            t_hidden_flat.dtype
        ).T
        t_logits = teacher.softcap(t_logits_proj)

        # Stop gradients — teacher predictions are constants for student
        t_logits_stopped = mx.stop_gradient(t_logits)
        mx.eval(t_logits_stopped)

        # ── STEP 3: Student learns from teacher + real data ───
        # Student loss = alpha*CE + (1-alpha)*KL
        def student_loss_fn(x_in, y_in):
            return model.loss(
                x_in,
                y_in,
                teacher_logits=t_logits_stopped,
                temperature=args.temperature,
                alpha=args.alpha,
            )

        s_loss, s_grads = nn.value_and_grad(model, student_loss_fn)(x, y)
        student_loss_accum = student_loss_accum + s_loss.astype(mx.float32) * scale
        student_grad_accum = accumulate_flat_grads(student_grad_accum, s_grads, scale)

        if args.mlx_eager_eval:
            mx.eval(teacher_loss_accum, teacher_grad_accum)
            mx.eval(student_loss_accum, student_grad_accum)

    return (
        teacher_loss_accum,
        student_loss_accum,
        tree_unflatten(list(teacher_grad_accum.items())),
        tree_unflatten(list(student_grad_accum.items())),
    )
...
def main() -> None:
    ...
    teacher_opt = SplitOptimizers(teacher, args)

    # ── STUDENT model (smaller, gets submitted) ──
    model = GPT(
        vocab_size=args.vocab_size,
    ...
    # ── Student compiled functions (unchanged) ──
    ...
    # ── Teacher compiled functions ──
    # Teacher uses normal CE loss (learns from real data)
    compiled_teacher_loss_and_grad = mx.compile(
        nn.value_and_grad(teacher, lambda x, y: teacher.loss(x, y)),
        inputs=teacher.state,
        outputs=teacher.state,
    )

    # Teacher forward only (extract logits to pass to student)
    compiled_teacher_forward = mx.compile(
        lambda x: teacher(x),
        inputs=teacher.state,
        outputs=teacher.state,
    )
    ...
    # ── NEW: distillation info ──
    n_teacher_params = sum(
        int(np.prod(p.shape)) for _, p in tree_flatten(teacher.parameters())
    )
    log(
        f"distillation:enabled "
        f"teacher_layers:{args.teacher_num_layers} "
        f"teacher_dim:{args.teacher_model_dim} "
        f"teacher_params:{n_teacher_params}"
    )
    log(
        f"distillation_settings: "
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            teacher_accum: dict[str, mx.array] | None = None
            warmup_loss = mx.array(0.0, dtype=mx.float32)
            grad_scale = 1.0 / args.grad_accum_steps

            for _ in range(args.grad_accum_steps):
                # Prime teacher graph
                t_wloss, t_wgrads = loss_and_grad_chunked(
                    args, train_loader, compiled_teacher_loss_and_grad
                )
                teacher_accum = accumulate_flat_grads(
                    teacher_accum, t_wgrads, grad_scale
                )
                # Prime student graph
    ...
    while True:
        ...
        # distill_step reads tokens ONCE and trains both models
        # on the exact same batch of tokens

        teacher_loss_total = mx.array(0.0, dtype=mx.float32)
        student_loss_total = mx.array(0.0, dtype=mx.float32)
        teacher_accum: dict[str, mx.array] | None = None
        student_accum: dict[str, mx.array] | None = None
        grad_scale = 1.0 / args.grad_accum_steps

        for _ in range(args.grad_accum_steps):
            t_loss, s_loss, t_grads, s_grads = distill_step(
                args,
                train_loader,
                teacher,
                model,
            ...
            teacher_loss_total = teacher_loss_total + t_loss * grad_scale
            student_loss_total = student_loss_total + s_loss * grad_scale
            teacher_accum = accumulate_flat_grads(teacher_accum, t_grads, grad_scale)
            student_accum = accumulate_flat_grads(student_accum, s_grads, grad_scale)
            if args.mlx_eager_eval:
                mx.eval(teacher_loss_total, teacher_accum)
                mx.eval(student_loss_total, student_accum)

        # ── Update TEACHER ────────────────────────────────────────
        t_grads_tree = tree_unflatten(list(teacher_accum.items()))
        t_grads_tree = clip_grad_tree(t_grads_tree, args.grad_clip_norm)
        teacher_opt.step(teacher, t_grads_tree, step=step, lr_mul=lr_mul)

        # ── Update STUDENT ────────────────────────────────────────
        grads = tree_unflatten(list(student_accum.items()))
        grads = clip_grad_tree(grads, args.grad_clip_norm)
        train_loss_value = float(student_loss_total.item())
