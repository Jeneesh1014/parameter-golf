...
"""
Knowledge Distillation for OpenAI Parameter Golf Challenge
Teacher-Student training: Teacher (large, never submitted) guides Student (small, submitted)
Based on train_gpt_mlx.py — Mac/Apple Silicon version
"""
...
class Hyperparameters:
    ...
    # STUDENT model (smaller — this gets submitted)
    ...
    # ─── DISTILLATION: Teacher model (bigger, never submitted) ───
...
def distillation_loss(
    ...
    """
    Combines two learning signals for the student:
      1. CE loss  — learn from real next tokens (ground truth)
      2. KL loss  — learn from teacher soft predictions

    Args:
        student_logits : [N, vocab_size]  raw student outputs
        teacher_logits : [N, vocab_size]  raw teacher outputs
        targets        : [N]              real next token ids
        temperature    : float            higher = softer distributions
        alpha          : float            weight on CE (1-alpha = KL weight)
    """

    # ── PART 1: Student vs real labels ──────────────────────────
    ...
    # ── PART 2: Student vs teacher soft labels ───────────────────
    # Temperature softens distributions so student learns
    # HOW wrong each wrong token is, not just which is right
    ...
    # Multiply by T^2 per Hinton et al. 2015
    # Keeps gradient magnitudes balanced with CE loss
...
class GPT(nn.Module):
    ...
    def loss(
        ...
        """
        PATH A: teacher_logits is None  → normal cross entropy (original behavior)
        PATH B: teacher_logits provided → distillation loss (CE + KL)
        """
        x = self(input_ids).reshape(-1, self.tok_emb.weight.shape[1])
        y = target_ids.reshape(-1)

        # ── PATH A: No teacher — original behavior ───────────────
...
def distill_step(
    ...
    For each chunk of tokens:
      1. READ tokens ONCE — teacher and student see identical batch
      2. TEACHER learns from real labels (normal CE loss)
      3. GET teacher logits — stop gradient so teacher does not
         receive gradients from the student loss path
      4. STUDENT learns from real labels + teacher soft labels

    Returns:
      teacher_loss : scalar loss tensor for teacher
      student_loss : scalar loss tensor for student
      teacher_grads: gradient tree for teacher optimizer
      student_grads: gradient tree for student optimizer
    """
    # Use FULL batch tokens — distill_step handles all accumulation
    # No outer loop in main() — one call = one complete optimizer step
    ...
    for chunk_tokens in chunk_sizes:
        ...
        # ── READ TOKENS ONCE ──────────────────────────────────────
        # Both teacher and student use this exact same x, y
        x, y = train_loader.next_batch(chunk_tokens, args.train_seq_len)
        scale = float(y.size) / total_tokens

        # ── TEACHER: learn from real data ─────────────────────────
        # Normal CE loss — teacher gets smarter each step
        ...
        # ── TEACHER LOGITS: get soft predictions ──────────────────
        # Forward pass only — extract what teacher thinks about x
        ...
        # STOP GRADIENTS — teacher logits are constants for the student
        # Student loss cannot flow back into teacher parameters here
        t_logits_stopped = mx.stop_gradient(t_logits)
        mx.eval(t_logits_stopped)

        # ── STUDENT: learn from teacher + real labels ─────────────
        # Loss = alpha * CE(student, real) + (1-alpha) * KL(student, teacher)
        def student_loss_fn(x_in: mx.array, y_in: mx.array) -> mx.array:
            return model.loss(
                x_in,
                y_in,
...
def main() -> None:
    ...
    if not args.tie_embeddings:
        raise NotImplementedError("train_gpt_distill_mlx.py only supports tied embeddings")
    ...
    # Student: eval only (used for validation)
    ...
    # Student: train (used inside distill_step via nn.value_and_grad)
    ...
    # Teacher: forward only (extract logits for student)
    ...
    n_teacher_params = sum(int(np.prod(p.shape)) for _, p in tree_flatten(teacher.parameters()))
    ...
    log(f"distillation_settings: temperature:{args.temperature} alpha:{args.alpha}")
    ...
    log(
        ...
        f"layers:{args.num_layers} dim:{args.model_dim} "
    ...
    log(
        ...
        f"scalar_lr:{args.scalar_lr} muon_momentum:{args.muon_momentum} "
    ...
    if args.warmup_steps > 0:
        for warmup_step in range(args.warmup_steps):
            # Use distill_step to prime both teacher and student graphs
            # This is the EXACT same code path as real training
            # so MLX compiles and caches the right graph shapes
            _, s_wloss, _, _ = distill_step(
                args,
                train_loader,
                teacher,
                model,
        ...
        # Prime the standalone eval graph (used for validation)
        val_batch_tokens = args.val_batch_size // args.grad_accum_steps
        if val_batch_tokens < args.train_seq_len:
            raise ValueError(
                "VAL_BATCH_SIZE must provide at least one sequence; "
        ...
        # Reset loader so real training starts from token 0
        train_loader = TokenLoader(
    ...
    while True:
        ...
        # on the exact same batch — no outer accumulation loop needed
        ...
        if args.mlx_eager_eval:
            mx.eval(t_loss, s_loss)

        # ── Update TEACHER ────────────────────────────────────────────────────
        t_grads_tree = clip_grad_tree(t_grads, args.grad_clip_norm)
        teacher_opt.step(teacher, t_grads_tree, step=step, lr_mul=lr_mul)

        # ── Update STUDENT ────────────────────────────────────────────────────
        s_grads_tree = clip_grad_tree(s_grads, args.grad_clip_norm)
        train_loss_value = float(s_loss.item())
        opt.step(model, s_grads_tree, step=step, lr_mul=lr_mul)
    ...
    # Quantize student → int8 + zlib → must be under 16MB
    ...
    # Validate quantized model by loading it back and running eval
